...
@section('page-subtitle', 'Kelola data mahasiswa dari sisi akademik')
...
<style>
    ...
    .mhs-cell .avatar-sm { width:36px; height:36px; border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:12px; font-weight:700; color:#fff; flex-shrink:0; }
                ...
                <tr>
                    ...
                    <td>
                        <div class="action-group">
                            ...
                            <form action="{{ route('akademik.mahasiswa.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Yakin hapus data mahasiswa ini? Akun user terkait juga akan dihapus.')">
                                @csrf @method('DELETE')
...
<div class="modal-overlay" id="detailModal"><div class="modal-box">
    ...
    <div class="modal-actions">
        <button type="button" class="btn-cancel" onclick="document.getElementById('detailModal').classList.remove('active')">Tutup</button>
    </div>
</div></div>
@endsection
...
function openEdit(id) {
    ...
    if (!m) return;
    document.getElementById('editForm').action = '/akademik/mahasiswa/' + id;
