@extends('layouts.app')
@section('title', 'Master Data Jurusan - SIDU')
@section('page-title', 'Master Data — Jurusan')
...
<div class="content-card">
    <div class="content-card-header crud-toolbar">
        <h3>🏛️ Daftar Jurusan</h3>
        <button class="btn-add" onclick="document.getElementById('addModal').classList.add('active')">➕ Tambah Jurusan</button>
    </div>
    <div class="table-container">
        <table class="data-table">
            <thead><tr><th>No</th><th>Kode</th><th>Nama Jurusan</th><th>Jumlah Prodi</th><th>Aksi</th></tr></thead>
            <tbody>
                @forelse($data as $i => $item)
                <tr>
                    <td>{{ $i+1 }}</td>
                    <td><span class="badge badge-info">{{ $item->kode_jurusan }}</span></td>
                    <td style="font-weight:600">{{ $item->nama_jurusan }}</td>
                    <td>{{ $item->prodi_count }} Prodi</td>
                    <td>
                        <div class="action-group">
                            <button class="btn-action btn-edit" onclick="openEdit({{ $item->id }},'{{ $item->kode_jurusan }}','{{ addslashes($item->nama_jurusan) }}')">✏️</button>
                            <form action="{{ route('akademik.master-data.jurusan.destroy',$item->id) }}" method="POST" onsubmit="return confirm('Yakin hapus jurusan ini?')">@csrf @method('DELETE')<button type="submit" class="btn-action btn-delete">🗑️</button></form>
                        </div>
                    </td>
                </tr>
                @empty
                <tr><td colspan="5" style="text-align:center;color:var(--text-muted);padding:40px">Belum ada data jurusan</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
...
<div class="modal-overlay" id="addModal"><div class="modal-box">
    <div class="modal-title">🏛️ Tambah Jurusan</div>
    <form action="{{ route('akademik.master-data.jurusan.store') }}" method="POST">@csrf
        <div class="form-group"><label>Kode Jurusan *</label><input type="text" name="kode_jurusan" class="form-control" placeholder="JTI" required></div>
        <div class="form-group"><label>Nama Jurusan *</label><input type="text" name="nama_jurusan" class="form-control" placeholder="Teknologi Informasi" required></div>
...
<div class="modal-overlay" id="editModal"><div class="modal-box">
    <div class="modal-title">✏️ Edit Jurusan</div>
    <form id="editForm" method="POST">@csrf @method('PUT')
        <div class="form-group"><label>Kode Jurusan *</label><input type="text" name="kode_jurusan" id="e_kode" class="form-control" required></div>
        <div class="form-group"><label>Nama Jurusan *</label><input type="text" name="nama_jurusan" id="e_nama" class="form-control" required></div>
...
function openEdit(id,kode,nama){
    document.getElementById('editForm').action='/akademik/master-data/jurusan/'+id;
