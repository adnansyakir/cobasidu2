...
@section('title', 'Profile Keuangan - SIDU')
@section('page-title', 'Profile Keuangan')
...
<style>
    ...
    .profile-avatar-wrapper .avatar-badge:hover { background:var(--purple-600); transform:scale(1.1); }
    .profile-photo-card h4 { font-size:18px; font-weight:700; margin-bottom:4px; }
    .role-label { display:inline-flex; align-items:center; gap:6px; padding:4px 14px; border-radius:20px; font-size:12px; font-weight:600; background:var(--purple-50); color:var(--purple-600); border:1px solid var(--purple-200); margin-bottom:16px; }
    ...
    .btn-upload-photo:hover { transform:translateY(-2px); box-shadow:0 6px 20px rgba(139,92,246,0.4); }
    ...
    .btn-save:hover { transform:translateY(-2px); box-shadow:0 8px 24px rgba(139,92,246,0.4); }
...
<div class="profile-grid">
    <div class="profile-photo-card">
        ...
        <span class="role-label">🔹 {{ ucfirst($user->getRoleName()) }}</span>
        <p class="photo-hint">{{ $user->email }}</p>
        <div class="photo-actions">
            <form action="{{ route('keuangan.profile.update') }}" method="POST" enctype="multipart/form-data" id="photoForm">
                @csrf @method('PUT')
                ...
                <button type="button" class="btn-upload-photo" onclick="document.getElementById('foto_input').click()">📸 Upload Foto Baru</button>
            </form>
            @if($user->foto_profil)
            <form action="{{ route('keuangan.profile.delete-photo') }}" method="POST">
                @csrf @method('DELETE')
                <button type="submit" class="btn-delete-photo" onclick="return confirm('Hapus foto profil?')">🗑️ Hapus Foto</button>
            </form>
            @endif
    ...
    <div>
        <div class="profile-form-card">
            ...
            <form action="{{ route('keuangan.profile.update') }}" method="POST" enctype="multipart/form-data">
                @csrf @method('PUT')
                <div class="form-row">
                    <div class="form-group">
                        <label>Nama Lengkap</label>
                        <input type="text" name="nama_lengkap" class="form-control @error('nama_lengkap') is-invalid @enderror" value="{{ old('nama_lengkap', $user->nama_lengkap) }}" placeholder="Masukkan nama lengkap">
                ...
                <div class="form-row">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="no_telepon" class="form-control @error('no_telepon') is-invalid @enderror" value="{{ old('no_telepon', $user->no_telepon) }}" placeholder="08xxxxxxxxxx">
                ...
                <div class="form-row single">
                    <div class="form-group">
                        <label>Alamat</label>
                        <textarea name="alamat" class="form-control @error('alamat') is-invalid @enderror" placeholder="Masukkan alamat lengkap">{{ old('alamat', $user->alamat) }}</textarea>
        ...
        <div class="password-card">
            ...
            <form action="{{ route('keuangan.profile.password') }}" method="POST">
                @csrf @method('PUT')
                <div class="form-row single">
                    <div class="form-group">
