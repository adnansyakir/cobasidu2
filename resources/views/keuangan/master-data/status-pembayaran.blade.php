@extends('layouts.app')
@section('title', 'Status Pembayaran - SIDU')
@section('page-title', 'Master Data — Status Pembayaran')
@section('page-subtitle', 'Kelola status pembayaran untuk transaksi keuangan')
...
<div class="content-card">
    <div class="content-card-header crud-toolbar">
        <h3>✅ Daftar Status Pembayaran</h3>
        <button class="btn-add" onclick="document.getElementById('addModal').classList.add('active')">➕ Tambah Status</button>
    </div>
    <div class="table-container">
        <table class="data-table">
            <thead><tr><th>No</th><th>Nama Status</th><th>Keterangan</th><th>Aksi</th></tr></thead>
            <tbody>
                @forelse($data as $i => $item)
                <tr>
                    <td>{{ $i + 1 }}</td>
                    <td style="font-weight:600">{{ $item->nama_status }}</td>
                    <td style="color:var(--text-secondary)">{{ $item->keterangan ?? '-' }}</td>
                    <td>
                        <div class="action-group">
                            <button class="btn-action btn-edit" onclick="openEdit({{ $item->id }},'{{ addslashes($item->nama_status) }}','{{ addslashes($item->keterangan) }}')">✏️</button>
                            <form action="{{ route('keuangan.master-data.status-pembayaran.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Yakin hapus status ini?')">
                                @csrf @method('DELETE')
                ...
                @empty
                <tr><td colspan="4" style="text-align:center;color:var(--text-muted);padding:40px">Belum ada data status pembayaran</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
...
<div class="modal-overlay" id="addModal"><div class="modal-box">
    <div class="modal-title">✅ Tambah Status Pembayaran</div>
    <form action="{{ route('keuangan.master-data.status-pembayaran.store') }}" method="POST">@csrf
        <div class="form-group"><label>Nama Status *</label><input type="text" name="nama_status" class="form-control" placeholder="Lunas" required></div>
        <div class="form-group"><label>Keterangan</label><textarea name="keterangan" class="form-control" placeholder="Deskripsi status pembayaran..."></textarea></div>
...
<div class="modal-overlay" id="editModal"><div class="modal-box">
    <div class="modal-title">✏️ Edit Status Pembayaran</div>
    <form id="editForm" method="POST">@csrf @method('PUT')
        <div class="form-group"><label>Nama Status *</label><input type="text" name="nama_status" id="e_nama_status" class="form-control" required></div>
...
function openEdit(id, nama, ket) {
    document.getElementById('editForm').action = '/keuangan/master-data/status-pembayaran/' + id;
    document.getElementById('e_nama_status').value = nama;
