@extends('layouts.app')
@section('title', 'Sumber Pembiayaan - SIDU')
@section('page-title', 'Master Data — Sumber Pembiayaan')
@section('page-subtitle', 'Kelola sumber pembiayaan mahasiswa')
...
<div class="content-card">
    <div class="content-card-header crud-toolbar">
        <h3>🏦 Daftar Sumber Pembiayaan</h3>
        <button class="btn-add" onclick="document.getElementById('addModal').classList.add('active')">➕ Tambah Sumber</button>
    </div>
    <div class="table-container">
        <table class="data-table">
            <thead><tr><th>No</th><th>Nama Sumber</th><th>Aksi</th></tr></thead>
            <tbody>
                @forelse($data as $i => $item)
                <tr>
                    <td>{{ $i + 1 }}</td>
                    <td style="font-weight:600">{{ $item->nama_sumber }}</td>
                    <td>
                        <div class="action-group">
                            <button class="btn-action btn-edit" onclick="openEdit({{ $item->id }},'{{ addslashes($item->nama_sumber) }}')">✏️</button>
                            <form action="{{ route('keuangan.master-data.sumber-pembiayaan.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Yakin hapus sumber pembiayaan ini?')">
                                @csrf @method('DELETE')
                ...
                @empty
                <tr><td colspan="3" style="text-align:center;color:var(--text-muted);padding:40px">Belum ada data sumber pembiayaan</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
...
<div class="modal-overlay" id="addModal"><div class="modal-box">
    <div class="modal-title">🏦 Tambah Sumber Pembiayaan</div>
    <form action="{{ route('keuangan.master-data.sumber-pembiayaan.store') }}" method="POST">@csrf
        <div class="form-group"><label>Nama Sumber *</label><input type="text" name="nama_sumber" class="form-control" placeholder="KIP-Kuliah" required></div>
...
<div class="modal-overlay" id="editModal"><div class="modal-box">
    <div class="modal-title">✏️ Edit Sumber Pembiayaan</div>
    <form id="editForm" method="POST">@csrf @method('PUT')
        <div class="form-group"><label>Nama Sumber *</label><input type="text" name="nama_sumber" id="e_nama_sumber" class="form-control" required></div>
        <div class="modal-actions">
...
function openEdit(id, nama) {
    document.getElementById('editForm').action = '/keuangan/master-data/sumber-pembiayaan/' + id;
    document.getElementById('e_nama_sumber').value = nama;
