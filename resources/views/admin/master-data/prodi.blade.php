                ...
                <tr>
                    ...
                    <td>
                        <div class="action-group">
                            ...
                            <form action="{{ route('admin.master-data.prodi.destroy',$item->id) }}" method="POST" onsubmit="return confirm('Yakin hapus prodi ini?')">@csrf @method('DELETE')<button type="submit" class="btn-action btn-delete">🗑️</button></form>
                        </div>
                    </td>
                </tr>
                @empty
...
<div class="modal-overlay" id="addModal"><div class="modal-box">
    ...
    <form action="{{ route('admin.master-data.prodi.store') }}" method="POST">@csrf
...
function openEdit(id,kode,nama,jenjang,jurusanId){
    document.getElementById('editForm').action='/admin/master-data/prodi/'+id;
