@extends('layouts.app')
@section('title', 'Master Data Role - SIDU')
@section('page-title', 'Master Data — Role')
@section('page-subtitle', 'Kelola role pengguna')
...
<style>
    ...
    .modal-box { background:#fff; border-radius:18px; padding:28px; width:100%; max-width:420px; box-shadow:0 20px 60px rgba(0,0,0,0.15); animation:fadeInUp 0.3s ease; }
    ...
    .role-icon { width:36px; height:36px; border-radius:10px; display:flex; align-items:center; justify-content:center; font-size:18px; }
    .role-icon.admin { background:#fef3c7; }
    .role-icon.keuangan { background:#d1fae5; }
    .role-icon.akademik { background:#dbeafe; }
    .role-icon.direktur { background:#ede9fe; }
    .role-icon.mahasiswa { background:#fce7f3; }
</style>
@endsection
...
<div class="content-card">
    <div class="content-card-header crud-toolbar">
        <h3>🔑 Daftar Role</h3>
        <button class="btn-add" onclick="document.getElementById('addModal').classList.add('active')">➕ Tambah Role</button>
    </div>
    <div class="table-container">
        <table class="data-table">
            <thead><tr><th>No</th><th>Nama Role</th><th>Jumlah User</th><th>Aksi</th></tr></thead>
            <tbody>
                @forelse($data as $i => $item)
                @php $icons=['admin'=>'👑','keuangan'=>'💰','akademik'=>'📘','direktur'=>'🏢','mahasiswa'=>'🎓']; @endphp
                <tr>
                    <td>{{ $i+1 }}</td>
                    <td>
                        <div style="display:flex;align-items:center;gap:10px">
                            <div class="role-icon {{ $item->nama_role }}">{{ $icons[$item->nama_role] ?? '🔹' }}</div>
                            <strong>{{ ucfirst($item->nama_role) }}</strong>
                        </div>
                    </td>
                    <td>{{ $item->users_count }} User</td>
                    <td>
                        <div class="action-group">
                            <button class="btn-action btn-edit" onclick="openEdit({{ $item->id }},'{{ addslashes($item->nama_role) }}')">✏️</button>
                            <form action="{{ route('admin.master-data.role.destroy',$item->id) }}" method="POST" onsubmit="return confirm('Yakin hapus role ini?')">@csrf @method('DELETE')<button type="submit" class="btn-action btn-delete">🗑️</button></form>
                        </div>
                    </td>
                </tr>
                @empty
                <tr><td colspan="4" style="text-align:center;color:var(--text-muted);padding:40px">Belum ada data role</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
...
<div class="modal-overlay" id="addModal"><div class="modal-box">
    <div class="modal-title">🔑 Tambah Role</div>
    <form action="{{ route('admin.master-data.role.store') }}" method="POST">@csrf
        <div class="form-group"><label>Nama Role *</label><input type="text" name="nama_role" class="form-control" placeholder="contoh: staff" required></div>
...
<div class="modal-overlay" id="editModal"><div class="modal-box">
    <div class="modal-title">✏️ Edit Role</div>
    <form id="editForm" method="POST">@csrf @method('PUT')
        <div class="form-group"><label>Nama Role *</label><input type="text" name="nama_role" id="e_nama" class="form-control" required></div>
...
function openEdit(id,nama){
    document.getElementById('editForm').action='/admin/master-data/role/'+id;
